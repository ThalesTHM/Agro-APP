...
const BHCICADxARMChart = ({data}) => {
    const font = useFont(inter, 9);

    const {state, isActive} = useChartPressState({x: 0, y: {valorArm: 0, valorCad: 100}})
    ...
    const animatedText = useAnimatedProps(()=>{
        return{
            text: "ARM " + state.y.valorArm.value.value.toFixed(2).toString() + " mm - CAD " + state.y.valorCad.value.value.toFixed(2).toString() + " mm",
            defaultValue: ""
        }
    })
        ...
        <View style={{height: 400}}>
            ...
            <View style={{height: 100}}>
                <Text style={{ fontSize: 30, fontWeight: 'bold', color: "#000" }}>
                    ARM {data[0]['valorArm'].toFixed(2)} mm - CAD {data[0]['valorCad'].toFixed(2)} mm
                </Text>
                <Text>
                    {data[0]['data']}
                </Text>
...
export default BHCICADxARMChart