...
const BHCIPxARMChart = ({data}) => {
    ...
    const animatedText = useAnimatedProps(()=>{
        return{
            text: "P " + state.y.valorP.value.value.toFixed(2).toString() + " mm - ARM " + state.y.valorArm.value.value.toFixed(2).toString() + ' mm',
            defaultValue: ""
        }
    })
        ...
        <View style={{height: 400}}>
            ...
            <View style={{height: 100}}>
                <Text style={{ fontSize: 30, fontWeight: 'bold', color: "#000" }}>
                    P {data[0]['valorP'].toFixed(2)} mm - ARM {data[0]['valorArm'].toFixed(2)} mm
                </Text>
                <Text>
                    {data[0]['data']}
                </Text>
