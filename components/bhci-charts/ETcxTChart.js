...
const BHCIETcxTChart = ({data}) => {
    const font = useFont(inter, 9);

    const {state, isActive} = useChartPressState({x: 0, y: {valorEtc: 0, valorT: 0}})
    ...
    const animatedText = useAnimatedProps(()=>{
        return{
            text: "ETc " + state.y.valorEtc.value.value.toFixed(2).toString() + " mm - T. média " + state.y.valorT.value.value.toFixed(2).toString() + " °C",
            defaultValue: ""
        }
    })
        ...
        <View style={{height: 400}}>
            ...
            <View style={{height: 100}}>
                <Text style={{ fontSize: 20, fontWeight: 'bold', color: "#000" }}>
                    ETc {data[0]['valorEtc'].toFixed(2)} mm - T. média {data[0]['valorT'].toFixed(2)} °C
                </Text>
                <Text>
                    {data[0]['data']}
                </Text>
