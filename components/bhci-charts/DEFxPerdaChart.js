...
const BHCIDEFxPerdaChart = ({data}) => {
    const font = useFont(inter, 9);

    const {state, isActive} = useChartPressState({x: 0, y: {valorDef: 0, valorPerda: 0}})
    ...
    const animatedText = useAnimatedProps(()=>{
        return{
            text: "DEF " + state.y.valorDef.value.value.toFixed(2).toString() + " mm - Perda " + state.y.valorPerda.value.value.toFixed(2).toString() + "%",
            defaultValue: ""
        }
    })
...
export default BHCIDEFxPerdaChart