...
const BHSCADxARMChart = ({data}) => {
    ...
    const animatedText = useAnimatedProps(()=>{
        return{
            text: "ARM " + state.y.valorArm.value.value.toFixed(2).toString() + "% - CAD " + state.y.valorCad.value.value.toFixed(2).toString(),
            defaultValue: ""
        }
    })
        ...
        <View style={{height: 400}}>
            ...
            <View style={{height: 100}}>
                <Text style={{ fontSize: 30, fontWeight: 'bold', color: "#000" }}>
                    ARM {data[0]['valorArm'].toFixed(2)}% - CAD {data[0]['valorCad'].toFixed(2)}
                </Text>
                <Text>
                    {data[0]['data']}
                </Text>
...
export default BHSCADxARMChart