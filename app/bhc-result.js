...
export default function Bhc() {
  ...
  useEffect(()=>{
    ...
    fetch(`https://sisdagro.inmet.gov.br/sisdagro/app/monitoramento/bhc.json?dataPlantio=${tilthData.tilth_start_date}&culturaId=${tilthData.tilth_type}&estacaoId=4300121000621400001&soloId=${Math.trunc(tilthData.ground_type)}&cad=${cad}`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        setData(result.bhc)
    ...
    <View className='w-screen h-full bg-lightblue'>
      ...
      {option == 'Resumo' && (
        ...
        <View className='flex-row items-center justify-center'>
          ...
          <ExcelExportBtn title={`Balanço Hídrico de Cultivo`} data={excelData} />
        </View>
      </ScrollView>
      )}
      {option == 'T' && (
        ...
        {option == 'DEFxPERDA' && (
            <View className='w-full h-full'>
                ...
                <View className='justify-center items-center flex-col mt-5'>
                    ...
                    <View className='bg-[#FFFF00] rounded-lg w-36 items-center m-2'>
        ...
        {option == 'ARMxPERDA' && (
            <View className='w-full h-full'>
                ...
                />
                <View className='justify-center items-center flex-row mt-5'>
                    <View className='bg-[#006400] rounded-lg w-36 items-center m-2'>
                        <Text className='text-base text-white'>Perda acumulada</Text>
                    </View>
                    <View className='bg-[#0000FF] rounded-lg w-36 items-center m-2'>
